...
class Fees extends Admin_Controller {
  ...
  function setting(){
        $this->set_title("Payment Setting");
        $data = $this->includes;

         if($this->input->post('mastercardUpdate'))
         {
              $this->fees_model->upsert('mastercard_api',$this->input->post('mastercardApiJson'));
              $this->session->set_flashdata('alert','<div class="alert alert-success">Api info updated</div>');
         }

         if($this->input->post('cybersecureUpdate'))
         {
             $this->fees_model->upsert('cybersecure_api',$this->input->post('cybersecureApiJson'));
             $this->session->set_flashdata('alert','<div class="alert alert-success">Api info updated</div>');
         }

    $content_data['mastercardApiJson'] = $this->fees_model->getMasterCardApiDetail();
    $content_data['cybersecureApiJson'] = $this->fees_model->getCyberSecureApiDetail();
        // set content data
    $data['content'] = $this->load->view('admin/fees/setting',$content_data,TRUE);
    $this->load->view($this->template, $data);
  }

  function getBankList(){
     $this->set_title("Bank List");
     $data = $this->includes;
     $content_data['getBankList'] = $this->fees_model->getBankList(); 
     $data['content'] = $this->load->view('admin/fees/bank_list',$content_data,TRUE);
     $this->load->view($this->template, $data);
  }

  function putBankList(){
    $this->set_title("Add Bank");
    $data = $this->includes;
    if($this->input->post('addBank')){
       $this->fees_model->insertBankList(array(
         'bank_internal_id' => rand(999,99999),
         'bank_name' => $this->input->post('bank_name'),
         'status' => 1
       ));
       $this->session->set_flashdata('alert','<div class="alert alert-success">New Bank Added </div>');
         redirect('admin/fees/getBankList');
    }
    $data['content'] = $this->load->view('admin/fees/add_bank',$content_data="",TRUE);
    $this->load->view($this->template, $data);
  }
