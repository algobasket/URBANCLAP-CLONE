...
class Users_model extends CI_Model {
    ...
    {
      $sql = "
            SELECT SQL_CALC_FOUND_ROWS {$this->_db}.*,role.role_name as role_name
            FROM {$this->_db}
            LEFT JOIN role ON role.id = {$this->_db}.role_id
            WHERE deleted = '0'
        ";
  ...
  function getAllUserByRoleByServiceByAvailability($roleId,$serviceId,$status,$isAvailable){
    $query = $this->db->select('users.*,status.name as statusName')
                      ->from('users')
                      ->join('role','role.id = users.role_id','left')
                      ->join('status','status.id = users.status','left')
                      ->where('users.role_id = '.$roleId.' AND FIND_IN_SET('.$serviceId.',users.service_ids) > 0 AND users.isAvailable='.$isAvailable)
                      ->get();
    //echo $this->db->last_query();
    return $query->result_array();
  }

  function assignJobWorker($jobId,$data){
     $this->db->where('id',$jobId);
     $this->db->update('jobs',$data);
     return true;
  }

  function projectStatusUpdate($id,$data){
    $this->db->where('id',$id);  
    $this->db->update('jobs',$data);
    return true;
  }
   ...
   function getUserPaymentDetail($uid,$moduleName){
     $query = $this->db->query("SELECT json FROM payment_settings WHERE user_id = ? AND module_name=?",array($uid,$moduleName));
     return $query->result_array()[0]['json'];
   }

   function status($id){
     $query = $this->db->query("SELECT name FROM status WHERE id = ?",array($id));
     return $query->result_array()[0]['name'];
   }

   function bankName($id){
     $query = $this->db->query("SELECT bank_name FROM bank_list WHERE id = ?",array($id));
     return $query->result_array()[0]['bank_name'];
   }
